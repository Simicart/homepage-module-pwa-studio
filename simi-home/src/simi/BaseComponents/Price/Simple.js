    ...
    renderView=()=>{
        ...
        if (this.prices.has_special_price) {
            if (this.prices.show_ex_in_price !== null && this.prices.show_ex_in_price === 1) {
                ...
                price_excluding_tax = (
                    <div>{'Excl. Tax'}: {this.formatPrice(this.prices.minimalPrice.excl_tax_amount.value, this.prices.minimalPrice.amount.currency)}</div>
                );
                price_including_tax = (
                    <div>{'Incl. Tax'}: {this.formatPrice(this.prices.minimalPrice.amount.value, this.prices.minimalPrice.amount.currency)}</div>
                );
            } else {
            ...
            price_label = (
                <div>{'Regular Price'}: {this.formatPrice(this.prices.regularPrice.amount.value, false)} <span
